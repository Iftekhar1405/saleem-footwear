...
const Header = () => {
  ...
  useEffect(() => {
    const handleCartUpdate = () => {
      const savedCart = localStorage.getItem('cart');
      setCartLength(savedCart ? JSON.parse(savedCart).length : 0);
    };
  ...
  const [LikedLength, setLikedLength] = useState(() => {
    ...
    return savedLiked ? JSON.parse(savedLiked).length : 0;
  });

  useEffect(() => {
    const handleLikedUpdate = () => {
      const savedLiked = localStorage.getItem('Liked');
      setLikedLength(savedLiked ? JSON.parse(savedLiked).length : 0);
    };

    window.addEventListener('Liked-updated', handleLikedUpdate);

    // Clean up the event listener on component unmount
    return () => window.removeEventListener('Liked-updated', handleLikedUpdate);
  }, []);
